...
app = FastAPI(title="TestSentry Dashboard API", version="2.1.0")
...
def triage_test_endpoint(payload: dict):
    """
    ...
    if not error_msg:
        raise HTTPException(status_code=400, detail="error_msg is required for triage")
...
def main():
    import uvicorn
    uvicorn.run("testsentry.api:app", host="0.0.0.0", port=8088, reload=True)
